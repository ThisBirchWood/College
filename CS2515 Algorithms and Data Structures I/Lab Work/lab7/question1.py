...
# as they are specified, and they must receive return types as specified.
    ...
    def _recursive_search(self, search_string, node):
        if search_string in str(node._element):
            return node._element
        if node._next == None:
            return None
        else:
            return self._recursive_search(search_string, node._next)

    def recursive_search(self, search_string):
        """ Return the first movie which contains word anywhere as a substring."""
        item = self._recursive_search(search_string, self._first)
        if item == None:

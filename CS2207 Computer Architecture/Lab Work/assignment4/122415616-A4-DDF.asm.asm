...
###############################################################################
	.globl main
	.data
		cpu_use: .asciiz "\nCPU has iterated this many times: "
...
__interrupt: 
	...
	andi $k0, $k0, 65280	#isolate the pending bits
	srl $k0, $k0, 8		#shift 8 to get the actual value of the bits
	
	beq $k0, 1, __keyboard_interrupt	#1 = keyboard interrupt
...
__keyboard_interrupt:     	
 	lw $k0, 0xffff0004	#load the ascii value that was pressed
 	
 	li $v0, 11		#syscall 11 prints ascii char
 	move $a0, $k0
 	syscall 
